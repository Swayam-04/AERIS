...
class EngineSimulator:
    """
    Deterministic Aero-Piston Engine Telemetry Simulator.
    Simulates real-time telemetry generation across flight phases with controlled fault injection.
    """
    ...
    def step(self, dt_seconds: float = 1.0) -> TelemetryRecord:
        """
        Advance simulation by dt_seconds and return observed telemetry.
        """
        ...
        volts = expected.battery_volts + self.rng.gauss(0, 0.08)

        # Apply Controlled Fault Deviations based on active fault & severity
        if self.active_fault != FaultType.NONE:
            time_since_fault = max(0.0, self.elapsed_time - self.fault_start_time)
            # Ramp-up multiplier for realistic fault progression over 10 seconds
            ramp = min(1.0, time_since_fault / 10.0) * self.fault_severity

            if self.active_fault == FaultType.MISFIRE:
                rpm -= 320.0 * ramp
                vibration += 1.85 * ramp
                egt -= 110.0 * ramp  # Unburnt fuel cools EGT in misfiring cylinder
                fuel_flow += 3.2 * ramp

            elif self.active_fault == FaultType.INJECTOR_ABNORMALITY:
                egt += 145.0 * ramp  # Lean mixture raises EGT drastically
                cht += 28.0 * ramp
                fuel_flow -= 4.5 * ramp
                vibration += 0.45 * ramp

            elif self.active_fault == FaultType.OIL_PRESSURE_LOSS:
                oil_press -= 38.0 * ramp  # Severe drop in oil pressure
                oil_temp += 42.0 * ramp   # Friction thermal buildup
                vibration += 0.65 * ramp
                ...
                vibration += 3.2 * ramp  # Imbalance or bearing wear
                rpm += self.rng.gauss(0, 45.0) * ramp

            elif self.active_fault == FaultType.SENSOR_DRIFT:
                cht += 45.0 * ramp  # Artificial sensor reading drift
                egt -= 35.0 * ramp
